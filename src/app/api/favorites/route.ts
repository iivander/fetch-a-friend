...
export async function GET(req: Request) {
    try {
        ...
        const favorites = (await sql('SELECT dogId AS "dogId" FROM favorite WHERE userId = $1', [
            userId,
        ])) as { dogId: string }[];
        ...
        return NextResponse.json(
            { error: "Error adding favorite", details: errorMessage },
            { status: 500 }
        );
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        await sql("INSERT INTO favorite (userId, dogId) VALUES ($1, $2) ON CONFLICT DO NOTHING", [
            userId,
            dogId,
        ]);
